...
/**
 *
 * Close Program : used to close a Program by setting the closeDate.
 *
 * @author JM235353
 *
 */
public class CloseProgramTask extends AbstractAuthenticateDatabaseTask {


    private String URL;

    private Date m_closeDate = null;
    private ProgramJson[] m_programJson = null;


    public CloseProgramTask(AbstractDatabaseCallback callback, Date closeDate, ProgramJson[] programJson) {
        super(callback, new TaskInfo("Close Program "+programJson[0].getTitle(), false, null), TokenManager.TOKEN_EPIMS_SERVER);

        URL = DataManager.getServerURL()+"/api/programclose";
