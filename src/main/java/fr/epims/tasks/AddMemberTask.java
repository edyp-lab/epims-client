...
/**
 *
 * Fragment samples of a study
 *
 * @author JM235353
 *
 */
public class AddMemberTask extends AbstractAuthenticateDatabaseTask {
    ...
    public AddMemberTask(AbstractDatabaseCallback callback, ProgramJson[] programJson, List<ContactJson> contact) {
        super(callback, new TaskInfo("Add Member "+contact.get(0).getLastName()+" to "+programJson[0].getTitle(), false, null), TokenManager.TOKEN_EPIMS_SERVER);

        URL = DataManager.getServerURL()+"/api/programaddmember";
     ...
     */
    public AddMemberTask(AbstractDatabaseCallback callback, ProjectJson[] projectJson, List<ContactJson> contact) {
        super(callback, new TaskInfo("Add Member "+contact.get(0).getLastName()+" to "+projectJson[0].getTitle(), false, null), TokenManager.TOKEN_EPIMS_SERVER);

        URL = DataManager.getServerURL()+"/api/projectaddmember";
     ...
     */
    public AddMemberTask(AbstractDatabaseCallback callback, StudyJson[] studyJson, List<ContactJson> contact) {
        super(callback, new TaskInfo("Add Member "+contact.get(0).getLastName()+" to "+studyJson[0].getTitle(), false, null), TokenManager.TOKEN_EPIMS_SERVER);

        URL = DataManager.getServerURL()+"/api/studyaddmember";
    ...
    private boolean fetchSecuredDataForProgram(HttpEntity<String> entity, RestTemplate restTemplate) {
        ...
        try {
            ...
            for (ContactJson contact : m_contact) {
                keys.add(DataManager.getActorFromContactId(contact.getId()).getLogin());
            }

            HttpEntity<List<String>> requestEntity = new HttpEntity<>(keys, entity.getHeaders());

            // Send request with GET method, and Headers.
            ResponseEntity<ProgramJson> response = restTemplate.exchange(URL+"/"+m_programJson[0].getId(),
                    HttpMethod.POST, requestEntity, ProgramJson.class);  //new ParameterizedTypeReference<List<ProgramJson>>() {}
